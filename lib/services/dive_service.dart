...
class DiveService {
    ...
    _connectivitySubscription = _connectivity.onConnectivityChanged.listen(
      (List<ConnectivityResult> results) async {
        ...
        if (wasOffline && _isOnline) {
          debugPrint('Connection restored. Starting automatic sync...');
          _syncStatusController.add(SyncStatus.reconnected);
          syncPendingDives();
        } else if (!_isOnline) {
          debugPrint('Connection lost. Working in offline mode.');
  ...
  Future<DiveSession> createDiveSession(DiveSession session) async {
    ...
    if (_isOnline) {
      try {
        await _firestoreService.createDiveSession(newSession, newSession.userId);
        ...
        await _updateLocalSession(newSession);
        debugPrint('Dive session created and synced: ${newSession.id}');
      } catch (e) {
        debugPrint('Error syncing new dive to Firestore (will retry later): $e');
      }
    }
  ...
  Future<DiveSession> updateDiveSession(DiveSession session) async {
    ...
    if (_isOnline) {
      try {
        ...
        await _firestoreService.createDiveSession(updatedSession, updatedSession.userId);
        ...
        await _updateLocalSession(updatedSession);
        debugPrint('Dive session updated and synced: ${updatedSession.id}');
      } catch (e) {
        debugPrint('Error syncing updated dive to Firestore (will retry later): $e');
      }
    }
  ...
  Future<void> deleteDiveSession(String id, String userId) async {
    ...
    if (_isOnline) {
      try {
        ...
        debugPrint('Error deleting from Firestore (zombie record risk): $e');
      }
    }
  }
  ...
  Future<void> syncPendingDives() async {
    ...
    debugPrint('Found ${pendingSessions.length} pending dives to sync...');
    ...
    for (var session in pendingSessions) {
      try {
        ...
        successCount++;
        debugPrint('Synced pending dive: ${session.id}');
      } catch (e) {
        failCount++;
        debugPrint('Failed to sync dive ${session.id}: $e');
      }
    }
    ...
    if (failCount == 0) {
      _syncStatusController.add(SyncStatus.completed);
      debugPrint('✅ All $successCount dives synced successfully');
    } else {
      _syncStatusController.add(SyncStatus.error);
      debugPrint('⚠️ Sync completed with errors: $successCount succeeded, $failCount failed');
    }
  }
  ...
  Future<Map<String, dynamic>> getStatistics(String userId) async {
    await initialize();
    final userSessions = _sessions.where((session) => session.userId == userId).toList();
    return calculateStatistics(userSessions);
  }

  static Map<String, dynamic> calculateStatistics(List<DiveSession> userSessions) {
