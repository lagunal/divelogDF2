...
class ExportService {
  ...
  Future<void> exportDiveToPdf(DiveSession dive) async {
    ...
    final String fileName =
        'Dive_${_fileDateFormat.format(dive.horaEntrada)}.pdf';
    ...
    return pw.Container(
      ...
      child: pw.Column(
        ...
        children: [
          ...
          _buildRow('Buque/Instalación', dive.buque),
        ...
        children: [
          ...
          pw.Row(
            children: [
              pw.Expanded(
                  child: _buildRow(
                      'Entrada', _dateFormat.format(dive.horaEntrada))),
              pw.Expanded(
                  child:
                      _buildRow('Salida', _dateFormat.format(dive.horaSalida))),
            ],
          ),
          pw.Row(
            children: [
              pw.Expanded(
                  child: _buildRow('Prof. Max', '${dive.maximaProfundidad} m')),
              pw.Expanded(
                  child: _buildRow(
                      'Intervalo', '${dive.tiempoIntervaloSuperficie} min')),
            ],
          ),
          pw.Row(
            children: [
              pw.Expanded(
                  child: _buildRow('Tiempo Fondo', '${dive.tiempoFondo} min')),
              pw.Expanded(
                  child: _buildRow(
                      'Tiempo Total', '${dive.tiempoTotalInmersion} min')),
            ],
          ),
          if (dive.inicioDescompresion != null)
            _buildRow('Inicio Descomp.',
                _dateFormat.format(dive.inicioDescompresion!)),
          if (dive.descompresionCompleta != null)
            _buildRow('Fin Descomp.',
                _dateFormat.format(dive.descompresionCompleta!)),
        ],
      ),
    );
  }
      ...
      child: pw.Column(
        ...
        children: [
          ...
          pw.Row(
            children: [
              pw.Expanded(
                  child: _buildRow('T. Supervisión',
                      '${dive.tiempoSupervisionAcumulado} hrs')),
              pw.Expanded(
                  child: _buildRow(
                      'T. Buceo', '${dive.tiempoBuceoAcumulado} hrs')),
            ],
          ),
        ],
      ),
  ...
  pw.Widget _buildFooter(DiveSession dive) {
    return pw.Column(
      ...
      children: [
        ...
        pw.SizedBox(height: 20),
        pw.Text('Generado por DiveDataPro - Bitácora de Buceo',
  ...
  Future<void> exportDiveToCsv(DiveSession dive) async {
    ...
    // Share
    final String fileName =
        'Dive_${_fileDateFormat.format(dive.horaEntrada)}.csv';
  ...
  Future<void> exportDivesListToCsv(List<DiveSession> dives) async {
    ...
    // Share
    final String fileName =
        'Reporte_Buceos_${_fileDateFormat.format(DateTime.now())}.csv';
  ...
  Future<void> exportDivesListToPdf(List<DiveSession> dives) async {
        ...
        build: (pw.Context context) {
          return [
            ...
            pw.Padding(
              ...
              child: pw.Text('Total de inmersiones: ${dives.length}',
    ...
    final String fileName =
        'Reporte_Buceos_${_fileDateFormat.format(DateTime.now())}.pdf';
